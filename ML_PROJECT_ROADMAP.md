...
**Core pin/safe integration + generative no-ML regression — ✅ принят и
frozen.**

**Classical scaffold + portability — ✅ приняты.** Classical adapter принят на
commit `ff7ade3fc598685d2488e2cbbc08fd1b14fa084d`.

**Umbrella/superchat smoke — ✅ принят.** Multi-repo isolation и ожидаемый
sync-apply refusal закрыты commit
`11dff82d2c34a2deba09ae9596a5ab5f6099df65`. Automatic Core sync apply остаётся
deferred technical debt, а не выполненной возможностью.

**Следующее точное действие:** review lessons-promotion lifecycle, затем решить
Core v1.0 freeze. Dataset audit остаётся после миграции. Generative ML остаётся
на паузе; ML-команды не запускать.
...
### 4.1 План миграции — ⏳ основные portability gates приняты
...
7. **Bootstrap classical ML repo — ✅ принят.** `product-conversion-ml-case`
создан через Core с adapter `classical_ml`; accepted portability evidence
зафиксирован на `ff7ade3fc598685d2488e2cbbc08fd1b14fa084d`.
8. **Portability smoke без обучения моделей — ✅ принят.** Luna/Terra lifecycle,
self-contained validation и supervisor review завершены.
9. **Umbrella local project и superchat smoke — ✅ принят.** Read-only
обнаружение трёх repos, изоляция ledgers и controlled target-only write
подтверждены на `11dff82d2c34a2deba09ae9596a5ab5f6099df65`.
Automatic Core sync apply остаётся deferred technical debt; v0.1 поддерживает
только non-mutating dry-run.
10. **Lessons-promotion lifecycle — следующее точное действие.** Локальная
ошибка → project report → кандидат на общее правило → human review → Core
release → dry-run sync → adapter validation. Каждое правило классифицируется
как `core`, конкретный adapter или `personal/private — never sync`.
11. **Core v1.0 freeze — решение после lessons-promotion review.**
...
5. **Core pin/safe integration + generative no-ML regression — ✅ принят и
frozen.** Generative backlog остаётся на паузе.
6. **Classical scaffold + portability — ✅ приняты** на commit
`ff7ade3fc598685d2488e2cbbc08fd1b14fa084d`.
7. **Umbrella/superchat smoke — ✅ принят** на commit
`11dff82d2c34a2deba09ae9596a5ab5f6099df65`; automatic Core sync apply
зафиксирован как deferred technical debt.
8. **Следующее точное действие:** review lessons-promotion lifecycle.
9. Затем принять отдельное решение о Core v1.0 freeze.
10. Только после миграции выполнить dataset audit в classical ML repo.
11. Затем вернуться к отдельному human-gated решению по `Cats → all AFHQ transfer`; после него — Generative Training & Evaluation Playbook v1 и лишь затем task-specific skills.
12. Generative backlog (img2img, hires, sampler ablation, RAE, native 256, Inspector) остаётся сохранённым и paused до явного возврата к нему.
