...
**Ближайшее действие:** после clean rollback commit текущего milestone создать отдельный `human-in-the-loop-ml-orchestration` Core repo v0.1. Generative backlog остаётся на паузе; ML-команды не запускать.
...
4. **Отдельный Core repo v0.1 — следующий шаг после clean rollback commit текущего milestone.** Создать `human-in-the-loop-ml-orchestration` как самостоятельный шаблон с version manifest, schemas, templates, bootstrap/sync/validation tools и документацией lifecycle, multi-repo supervision и lessons-promotion.
