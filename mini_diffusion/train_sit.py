...
def save_checkpoint(path: Path, model, optimizer, ema, cfg, step: int, fingerprint: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({"architecture": "SiT-S/2 velocity v1", "model": model.state_dict(), "ema": ema.state_dict(), "optimizer": optimizer.state_dict(), "global_step": step, "config": cfg, "cache_fingerprint": fingerprint, "torch_rng_state": torch.get_rng_state(), "cuda_rng_state": torch.cuda.get_rng_state_all() if torch.cuda.is_available() else None, "numpy_rng_state": np.random.get_state(), "python_rng_state": random.getstate()}, path)
def resume_checkpoint(path: str, model, optimizer, ema, device, expected_cache_fingerprint: str | None = None) -> int:
...
def overfit_smoke(cfg: dict, updates: int) -> None:
    set_seed(cfg.get("seed", 123)); device, dtype, autocast = device_dtype(cfg); model = build_model(cfg).to(device); optimizer = torch.optim.AdamW(model.parameters(), lr=max(cfg["train"]["learning_rate"], 1e-3)); loader, _ = make_loader(cfg); x0, labels = next(iter(loader)); x0, labels = x0.to(device).float(), labels.to(device); noise, t = torch.randn_like(x0), torch.linspace(0.1, 0.9, x0.shape[0], device=device); xt, target = linear_interpolant(x0, noise, t); losses = []
...
def main() -> None:
    ...
    torch.backends.cudnn.benchmark = bool(cfg.get("performance", {}).get("cudnn_benchmark", False)); model = build_model(cfg).to(device); optimizer = build_optimizer(model, cfg); ema = EMA(model, cfg["train"]["ema_decay"], foreach=cfg.get("performance", {}).get("ema_foreach", False)); loader, payload = make_loader(cfg); step = resume_checkpoint(args.resume, model, optimizer, ema, device, cache_fingerprint(payload)) if args.resume else 0
    params = sum(p.numel() for p in model.parameters() if p.requires_grad); accum = cfg["train"].get("grad_accum_steps", 1); print(f"trainable_parameters: {params:,}\ndevice: {device}\ndtype: {dtype} autocast={autocast}\nbatch_size: {cfg['data']['batch_size']}\neffective_batch_size: {cfg['data']['batch_size'] * accum}\nvae_loaded_for_training: false")
    ...
    while step < max_steps:
        ...
        if step % cfg["train"].get("log_every", 1) == 0: writer.add_scalar("train/loss", total, step); pbar.set_postfix(loss=f"{total:.4f}")
        if step % cfg["train"].get("sample_every", 10**9) == 0: print(f"latent_preview_written: {write_latent_preview(model, ema, cfg, device, step)}")
        if step % cfg["train"].get("save_every", 100) == 0: save_checkpoint(latest, model, optimizer, ema, cfg, step, cache_fingerprint(payload)); print(f"checkpoint_written: {latest}")
    save_checkpoint(latest, model, optimizer, ema, cfg, step, cache_fingerprint(payload)); writer.close(); pbar.close()
